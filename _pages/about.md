---
permalink: /
title: ""
layout: default
---

<div class="profile-intro" id="about">
  <img src="{{ base_path }}/images/profile.png" alt="Taimoor Tariq" class="profile-avatar">
  <div class="profile-info">
    <h1 class="profile-name">Taimoor Tariq</h1>
    <div class="profile-title">Computer Science Ph.D. Candidate &middot; UIUC</div>
    <div class="profile-actions">
      <a href="https://scholar.google.com/citations?hl=en&view_op=list_works&gmla=AL3_zihWHxPU-3hG7XprbCVmSO7aIvGUjFRXQpikMhrEPvyEK0KMcUoVq4wB66d2purnNp3XEfx9GkOkMooRHw&user=WEVKtBAAAAAJ" target="_blank" rel="noopener noreferrer" class="pill-btn">
        <svg viewBox="0 0 24 24"><path d="M12 24a7 7 0 1 1 0-14 7 7 0 0 1 0 14zm0-24L0 9.5l4.838 3.94v5.474L12 22.5l7.162-3.586V13.44L24 9.5 12 0z"/></svg>
        Google Scholar
      </a>
      <a href="https://github.com/taimoortt" target="_blank" rel="noopener noreferrer" class="pill-btn">
        <svg viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg>
        GitHub
      </a>
      <a href="mailto:mttariq2@illinois.edu" class="pill-btn">
        <svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
        Email
      </a>
      <a href="https://www.linkedin.com/in/taimoor-tariq-315547213/" target="_blank" rel="noopener noreferrer" class="pill-btn">
        <svg viewBox="0 0 24 24"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.28 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.75M6.46 8.76a1.69 1.69 0 0 0 1.68-1.69c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .94.76 1.69 1.69 1.69m1.39 9.94v-8.37H5.07v8.37h2.78z"/></svg>
        LinkedIn
      </a>
    </div>
  </div>
</div>

<p>
  I am a PhD candidate in Computer Science at the <strong>University of Illinois Urbana-Champaign (UIUC)</strong>, advised by Professor <a href="http://radhikam.web.illinois.edu" target="_blank" rel="noopener noreferrer">Radhika Mittal</a>.
</p>

<p>
  My research focuses on <strong>resource management and coordination in multi-tenant networks</strong>, with an emphasis on balancing <strong>efficiency, fairness, and performance isolation</strong>.
</p>

<p>
  Prior to UIUC, I completed my undergraduate studies at <strong>Lahore University of Management Sciences (LUMS)</strong>, where I worked with Professor <a href="https://web.lums.edu.pk/~zafar/" target="_blank" rel="noopener noreferrer">Zafar Ayyub Qazi</a> on the design of control plane architecture for 5G deployments.
</p>

<h2 id="publications">Publications</h2>

<div class="pub-item">
  <div class="pub-title">Saving Private Cellular: The Coordination Vacuum in CBRS</div>
  <div class="pub-meta">
    <span class="pub-badge">HotMobile 2026</span>
  </div>
  <div class="pub-authors">
    <strong>Taimoor Tariq</strong>, Radhika Mittal
  </div>
  <div class="pub-links">
    <a href="{{ base_path }}/files/saving-private-cellular.pdf" target="_blank" class="pub-link">Paper</a>
    <a href="{{ base_path }}/files/saving-private-cellular-slides.pdf" target="_blank" class="pub-link">Slides</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">Performance Isolation for 5G RAN Slicing Across Multiple Interfering Cells</div>
  <div class="pub-meta">
    <span class="pub-badge">NINeS 2026</span>
  </div>
  <div class="pub-authors">
    <strong>Taimoor Tariq</strong>, Yongzhou Chen, Haitham Hassanieh, Radhika Mittal
  </div>
  <div class="pub-links">
    <a href="{{ base_path }}/files/radioninja.pdf" target="_blank" class="pub-link">Paper</a>
    <a href="{{ base_path }}/files/radioninja-slides.pdf" target="_blank" class="pub-link">Slides</a>
    <a href="https://vimeo.com/showcase/NINeS?video=1161794549" target="_blank" rel="noopener noreferrer" class="pub-link">Talk</a>
    <a href="https://github.com/taimoortt/RadioNinja" target="_blank" rel="noopener noreferrer" class="pub-link">Code</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">5G RAN Slicing with Load Balanced Handovers</div>
  <div class="pub-meta">
    <span class="pub-badge">arXiv 2025</span>
  </div>
  <div class="pub-authors">
    Yongzhou Chen, <strong>Taimoor Tariq</strong>, Haitham Hassanieh, Radhika Mittal
  </div>
  <div class="pub-links">
    <a href="https://arxiv.org/abs/2505.24295" target="_blank" rel="noopener noreferrer" class="pub-link">Paper</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">Centralized Traffic Engineering for Networked Farm Applications</div>
  <div class="pub-meta">
    <span class="pub-badge">IEEE/ACM SEC 2025</span>
  </div>
  <div class="pub-authors">
    Ammar Tahir, Yueshen Li, Jianli Jin, Changxin Zhang, Daniel Moon, Aganze Mihigo, <strong>Taimoor Tariq</strong>, Deepak Vasisht, Radhika Mittal
  </div>
  <div class="pub-links">
    <a href="{{ base_path }}/files/farmnetes.pdf" target="_blank" class="pub-link">Paper</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">Enabling Emerging Edge Applications Through a 5G Control Plane Intervention</div>
  <div class="pub-meta">
    <span class="pub-badge">ACM CoNEXT 2022</span>
  </div>
  <div class="pub-authors">
    Mukhtiar Ahmad, Muhammad Nawazish Ali, <strong>Taimoor Tariq</strong>, Basit Iqbal, Taqi Raza, Zafar Ayyub Qazi
  </div>
  <div class="pub-links">
    <a href="{{ base_path }}/files/cellclone.pdf" target="_blank" class="pub-link">Paper</a>
    <a href="https://github.com/nsgLUMS/cellclone" target="_blank" rel="noopener noreferrer" class="pub-link">Code</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">Neutrino: A Fast and Consistent Edge-based Cellular Control Plane</div>
  <div class="pub-meta">
    <span class="pub-badge">IEEE/ACM Transactions on Networking, 2022</span>
  </div>
  <div class="pub-authors">
    Mukhtiar Ahmad, Muhammad Nawazish Ali, <strong>Taimoor Tariq</strong>, Syed Usman Jafri, Adnan Abbass, Syeda Mashal Abbas, Basit Iqbal, Zartash Uzmi, Zafar Ayyub Qazi
  </div>
  <div class="pub-links">
    <a href="https://ieeexplore.ieee.org/document/9885031" target="_blank" rel="noopener noreferrer" class="pub-link">Paper</a>
    <a href="https://github.com/nsgLUMS/neutrino" target="_blank" rel="noopener noreferrer" class="pub-link">Code</a>
  </div>
</div>

<div class="pub-item">
  <div class="pub-title">Fast Failure Detectors for 5G Edge Deployments (Poster)</div>
  <div class="pub-meta">
    <span class="pub-badge">SOSP 2021 SRC</span>
  </div>
  <div class="pub-authors">
    <strong>Taimoor Tariq</strong>, Maha Kamal, Vafa Batool
  </div>
</div>

<h2 id="education">Education</h2>

<div class="timeline-list">
  <div class="timeline-item">
    <div class="timeline-content">
      <div class="timeline-title">University of Illinois Urbana-Champaign (UIUC)</div>
      <div class="timeline-subtitle">Ph.D. in Computer Science &middot; Advised by Prof. Radhika Mittal</div>
    </div>
    <div class="timeline-date">2022 &ndash; Present</div>
  </div>

  <div class="timeline-item">
    <div class="timeline-content">
      <div class="timeline-title">Lahore University of Management Sciences (LUMS)</div>
      <div class="timeline-subtitle">B.S. in Computer Science &middot; Advised by Prof. Zafar Ayyub Qazi</div>
    </div>
    <div class="timeline-date">2018 &ndash; 2022</div>
  </div>
</div>
